...
try {
    ...
    if (!empty($search_value)) {
        ...
        $logs_where_conditions[] = "(pi.item_name LIKE '%$search_esc%' OR sl.date_created LIKE '%$search_esc%')";
    }
    ...
    if ($logs_total_pages > 1) {
        ...
        $end = min($logs_total_pages, $logs_page + $neighborRange);

        // First page
        $active_class = (1 === (int)$logs_page) ? ' active' : '';
        $pagination_html .= '<li class="page-item' . $active_class . '">';
        $pagination_html .= '<a class="page-link" href="#" onclick="loadStockMovements(1); return false;">1</a>';
        $pagination_html .= '</li>';
        ...
        // Middle pages
        for ($i = $start; $i <= $end; $i++) {
            if ($i === 1 || $i === (int)$logs_total_pages) continue;
        ...
        // Trailing ellipsis
        if ($end < ($logs_total_pages - 1)) {
            $pagination_html .= '<li class="page-item disabled"><span class="page-link">&hellip;</span></li>';
        }

        // Last page
        if ($logs_total_pages > 1) {
            $active_class = ((int)$logs_total_pages === (int)$logs_page) ? ' active' : '';
            $pagination_html .= '<li class="page-item' . $active_class . '">';
            $pagination_html .= '<a class="page-link" href="#" onclick="loadStockMovements(' . $logs_total_pages . '); return false;">' . $logs_total_pages . '</a>';
            $pagination_html .= '</li>';
        }
