        ...
        <div class="d-flex align-items-center" style="height: 10px;">
          ...
          <div>
            <h5 class="modal-title mb-0 fw-bold" id="supplyModalLabel">
              <i class="fas fa-plus-circle me-2"></i>New Property Request
              <small class="text-white-50" style="font-size: 15px;">(Submit your property request for processing)</small>
            </h5>
          </div>
        </div>
          ...
          <div class="section-card mb-4" style="margin-top: -40px;">
            ...
            <div class="row g-3">
              ...
              <div class="col-md-3">
                ...
                <small class="ms-2">Leave blank if not applicable</small>
              </div>

              <div class="col-md-3">
                <!-- Purpose selection as checkboxes -->
                <div class="mb-2">
                  ...
                  <!-- Hidden input to submit combined value to backend as 'purpose' -->
                  <input type="hidden" name="purpose" id="purposeHidden" required>
                </div>
              </div>

              <div class="col-md-3">
                <div class="form-floating">
                  <input type="text" name="reason_for_transfer" class="form-control" id="reasonForTransfer" required>
                  <label for="reasonForTransfer">
                    <i class="fas fa-bullseye me-1"></i>Reason for Transfer <span class="text-danger">*</span>
                  </label>
                </div>
              </div>
            ...
            <div class="row g-3">
              <div class="col-md-6">
                <div class="form-floating">
                  <select name="category" class="form-select" id="categorySelect" required>
                    <option value="">Select Category for the Property</option>
                    <?php
                    // IMPORTANT: HTML does not support nested <optgroup>. To mimic the screenshot:
                    // 1) Render a disabled heading option "Assets"
                    // 2) Then render one <optgroup> per main category with its child options indented
                    if (isset($organized_categories) && !empty($organized_categories)) {
                        echo '<option value="" disabled class="heading-option">Assets</option>';
                        ...
                        echo '<option value="" disabled class="heading-option">Assets</option>';
                        echo '<optgroup label="Property and Equipment">';
                        echo '<option value="">&nbsp;&nbsp;School Building - BED Campus</option>';
                        echo '<option value="">&nbsp;&nbsp;School Building - Main Campus</option>';
                        echo '</optgroup>';
                        echo '<optgroup label="Office Equipment">';
                        echo '<option value="">&nbsp;&nbsp;Office Equipment - BED Campus</option>';
                        echo '</optgroup>';
                    }
                    ?>
                  </select>
              ...
              <div class="col-md-3">
                <div class="form-floating">
                  ...
                  <label for="typeInput">
                    <i class="fas fa-hashtag me-1"></i>Type <span class="text-danger">*</span>
                  </label>
                </div>
              </div>
...
<style>
  ...
  /* Visual heading inside the select to mimic a top-level group title */
  #categorySelect option.heading-option {
    font-weight: bold;
    color: #1a5f3c; /* primary green */
    text-align: center;
  }
  /* Some browsers dim disabled options; make the heading readable */
...
<script>
  document.addEventListener('DOMContentLoaded', function() {
    ...
    const purposeTmp = document.getElementById('purposeTemporary');
    const purposePerm = document.getElementById('purposePermanent');
    const purposeHidden = document.getElementById('purposeHidden');

    function updatePurposeHidden() {
      // Determine single selected value
      let selectedValue = '';
      if (purposeTmp && purposeTmp.checked) selectedValue = purposeTmp.value;
      if (purposePerm && purposePerm.checked) selectedValue = purposePerm.value;
      if (purposeHidden) purposeHidden.value = selectedValue;
      const hasSelection = !!selectedValue;
      [purposeTmp, purposePerm].forEach(el => {
        if (!el) return;
        el.classList.toggle('is-invalid', !hasSelection);
        el.classList.toggle('is-valid', hasSelection);
      });
      return hasSelection;
    }

    // Make checkboxes mutually exclusive (radio-like behavior)
    [purposeTmp, purposePerm].forEach(el => {
      if (!el) return;
      el.addEventListener('change', function() {
        if (this.checked) {
          // Uncheck the other checkbox
          const other = this === purposeTmp ? purposePerm : purposeTmp;
    ...
    if (modal) {
      modal.addEventListener('shown.bs.modal', function() {
        updateTotal();
        // Reset purpose selection visuals on open
        updatePurposeHidden();
    ...
    if (form) {
      form.addEventListener('submit', function(e) {
        ...
        // Ensure at least one purpose selected
        const ok = updatePurposeHidden();
        if (!ok) {
          e.preventDefault();
          // Focus first checkbox
          if (purposeTmp) purposeTmp.focus();
          return;
        }
