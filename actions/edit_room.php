...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Check if room already exists (excluding current room)
    $check_stmt = $conn->prepare("SELECT id FROM rooms_inventory WHERE building_name = ? AND room_number = ? AND id != ?");
    $check_stmt->bind_param("ssi", $building_name, $room_number, $room_id);
    $check_stmt->execute();
    ...
    $stmt = $conn->prepare("
        ...
        SET building_name = ?, room_number = ?, floor = ?, 
            fluorescent_light = ?, electric_fans_wall = ?, ceiling = ?, 
            chairs_mono = ?, steel = ?, plastic_mini = ?, teacher_table = ?, 
            black_whiteboard = ?, platform = ?, tv = ?
        WHERE id = ?
    ");
    
    $stmt->bind_param("sssiiiiiiiiiii", 
        $building_name, $room_number, $floor, $fluorescent_light, $electric_fans_wall, 
        $ceiling, $chairs_mono, $steel, $plastic_mini, $teacher_table, 
        $black_whiteboard, $platform, $tv, $room_id
    );

    if ($stmt->execute()) {
        if ($stmt->affected_rows > 0) {
            $_SESSION['message'] = "Room '$building_name - Room $room_number' has been updated successfully.";
        } else {
            $_SESSION['error'] = "No changes were made to the room.";
        }
    } else {
