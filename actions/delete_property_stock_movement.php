...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    try {
        ...
        $get_log_sql = "SELECT sl.*, i.current_stock 
                       ...
                       JOIN property_inventory i ON sl.inventory_id = i.inventory_id 
                       WHERE sl.log_id = ?";
        ...
        if ($new_current_stock < 0) {
            throw new Exception('Cannot delete movement: would result in negative inventory ($new_current_stock)');
        }
        
        // Delete the stock movement record
        $delete_sql = "DELETE FROM property_stock_logs WHERE log_id = ?";
