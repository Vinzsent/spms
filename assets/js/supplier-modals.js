...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const fieldMap = {
    ...
    status: "edit-status",
    "tax-identification-number": "edit-tax-identification-number",
    notes: "edit-notes",
  };
  ...
  async function populateProvincesAndCities(
    provinceSelect,
    citySelect,
    selectedProvince = "",
    selectedCity = "",
  ...
  editModal.addEventListener("show.bs.modal", async function (event) {
    ...
    const selectedCountry =
      button.getAttribute("data-country") || "Philippines";
  ...
  const viewFieldMap = {
    ...
    status: "view-status",
    "tax-identification-number": "view-tin",
    notes: "view-notes",
  };
      ...
      for (const [dataAttr, inputId] of Object.entries(viewFieldMap)) {
        ...
        if (dataAttr === "status") {
          const statusBadgeContainer =
            document.getElementById("view-status-badge");
          if (statusBadgeContainer) {
            const badgeClass =
              value.toLowerCase() === "active" ? "bg-success" : "bg-danger";
            statusBadgeContainer.innerHTML = `<span class="badge ${badgeClass}">${value}</span>`;
          }
        }
      }
    });
