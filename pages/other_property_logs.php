<?php
$pageTitle = 'Other Property Management';
...
$status_sql = "SELECT status, COUNT(*) as count FROM other_property_logs" . $campus_clause . " GROUP BY status";
...
$good_condition_props = [];
$needs_attention_props = [];
$decommissioned_props = [];

// Get good condition aircons (Operational + Working)
$good_sql = "SELECT * FROM other_property_logs WHERE status IN ('Operational', 'Working')"
...
if ($good_result) {
    while ($row = $good_result->fetch_assoc()) {
        $good_condition_props[] = $row;
    }
}

// Get aircons needing attention (Needs Repair + Under Maintenance)
$attention_sql = "SELECT * FROM other_property_logs WHERE status IN ('Needs Repair', 'Under Maintenance')"
    . ($campus_clause ? str_replace(' WHERE ', ' AND ', $campus_clause) : '')
    . " ORDER BY location, brand";
$attention_result = $conn->query($attention_sql);
if ($attention_result) {
    while ($row = $attention_result->fetch_assoc()) {
        $needs_attention_props[] = $row;
    }
}

// Get decommissioned aircons
$decom_sql = "SELECT * FROM other_property_logs WHERE status = 'Decommissioned'"
    . ($campus_clause ? str_replace(' WHERE ', ' AND ', $campus_clause) : '')
    . " ORDER BY location, brand";
$decom_result = $conn->query($decom_sql);
if ($decom_result) {
    while ($row = $decom_result->fetch_assoc()) {
        $decommissioned_props[] = $row;
    }
}
...
<?php if (!$isAjax): ?>
    ...
    <div class="main-content">
        <div class="content-header">
            <h1>Other Property Management</h1>
            <p>Track property location, monitor property condition, and manage property details</p>
        </div>
        ...
        <div class="stats-container">
            <div class="stat-card">
                ...
                <div class="stat-label">Total Property Units</div>
            </div>

            <div class="stat-card clickable" style="border-left: 4px solid #28a745; cursor: pointer;" data-bs-toggle="modal" data-bs-target="#goodConditionModal" title="Click to view good condition property items">
            ...
            <div class="stat-card clickable" style="border-left: 4px solid #ffc107; cursor: pointer;" data-bs-toggle="modal" data-bs-target="#needsAttentionModal" title="Click to view units needing attention">
            ...
            <div class="stat-card clickable" style="border-left: 4px solid #dc3545; cursor: pointer;" data-bs-toggle="modal" data-bs-target="#decommissionedModal" title="Click to view decommissioned items">
        ...
        <!-- Good Condition Properties Modal -->
                ...
                <div class="modal-content">
                    <div class="modal-header" style="background: linear-gradient(135deg, #28a745 0%, #20c997 100%); color: white;">
                        <h5 class="modal-title" id="goodConditionModalLabel">
                            <i class="fas fa-check-circle me-2"></i>Good Condition Properties
                        </h5>
                        <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <?php if (!empty($good_condition_props)): ?>
                            <div class="alert alert-success">
                                <i class="fas fa-check-circle me-2"></i>
                                <strong><?= count($good_condition_props) ?></strong> aircon unit(s) are in good working condition.
                            </div>
                            <div class="table-responsive">
                                <table class="table table-hover table-striped">
                                    ...
                                    <tbody>
                                        <?php foreach ($good_condition_props as $aircon): ?>
                                            <tr>
                                                ...
                                                <td>
                                                    <button class="btn btn-sm btn-primary view-property-details-btn"
                                                        title="View Details"
                                                        data-property-id="<?= (int)$aircon['id'] ?>"
                        ...
                        <?php else: ?>
                            <div class="text-center py-5">
                                ...
                                <h4>No Aircons</h4>
                                <p class="text-muted">No other_property_logs in good condition found.</p>
                            </div>
                        <?php endif; ?>
                    </div>
                    <div class="modal-footer">
                ...
                <div class="modal-content">
                    <div class="modal-header" style="background: linear-gradient(135deg, #ffc107 0%, #ff9800 100%); color: white;">
                        <h5 class="modal-title" id="needsAttentionModalLabel">
                            <i class="fas fa-exclamation-triangle me-2"></i>Properties Needing Attention
                        </h5>
                        <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <?php if (!empty($needs_attention_props)): ?>
                            <div class="alert alert-warning">
                                <i class="fas fa-exclamation-triangle me-2"></i>
                                <strong><?= count($needs_attention_props) ?></strong> aircon unit(s) need repair or maintenance attention.
                            </div>
                            <div class="table-responsive">
                                <table class="table table-hover table-striped">
                                    <thead class="table-warning">
                                    ...
                                    <tbody>
                                        <?php foreach ($needs_attention_props as $aircon): ?>
                                            <tr>
                                                <td><strong><?= htmlspecialchars($aircon['brand'] ?? 'N/A') ?></strong></td>
                                                <td><?= htmlspecialchars($aircon['model'] ?? 'N/A') ?></td>
                                                <td><?= htmlspecialchars($aircon['type'] ?? 'N/A') ?></td>
        ...
        <!-- Decommissioned Properties Modal -->
                ...
                <div class="modal-content">
                    <div class="modal-header" style="background: linear-gradient(135deg, #dc3545 0%, #c82333 100%); color: white;">
                        <h5 class="modal-title" id="decommissionedModalLabel">
                            <i class="fas fa-times-circle me-2"></i>Decommissioned Properties
                        </h5>
                        <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <?php if (!empty($decommissioned_props)): ?>
                            <div class="alert alert-danger">
                                <i class="fas fa-times-circle me-2"></i>
                                <strong><?= count($decommissioned_props) ?></strong> aircon unit(s) have been decommissioned and are out of service.
                            </div>
                            <div class="table-responsive">
                                <table class="table table-hover table-striped">
                                    <thead class="table-danger">
                                    ...
                                    <tbody>
                                        <?php foreach ($decommissioned_props as $aircon): ?>
                                            <tr>
                                                <td><strong><?= htmlspecialchars($aircon['brand'] ?? 'N/A') ?></strong></td>
                                                <td><?= htmlspecialchars($aircon['model'] ?? 'N/A') ?></td>
                                                <td><?= htmlspecialchars($aircon['type'] ?? 'N/A') ?></td>
        ...
        <div class="table-container">
            <div class="table-header">
                <h3> Other Property Logs List</h3>
                <div class="d-flex align-items-end gap-2">
                    <form method="GET" class="d-flex align-items-end gap-2 mb-0">
                        <div class="search-input">
                            <label for="search" class="form-label mb-0 text-white">Search Property</label>
                        ...
                        <div class="pt-4">
                            ...
                            <?php if (!empty($search_term) || !empty($sy_inv_raw) || !empty($campus_raw)): ?>
                                <a href="other_property_logs.php" class="btn btn-outline-light ms-2">
                                    <i class="fas fa-times"></i> Clear
                                </a>
                            <?php endif; ?>
                        </div>
                    </form>
                    <a class="btn btn-success" href="../actions/export_other_property.php?search=<?= urlencode($search_term) ?>&sy_inv=<?= urlencode($sy_inv_raw) ?>&campus=<?= urlencode($campus_raw) ?>">
            ...
            $count_sql = "SELECT COUNT(*) as total FROM other_property_logs i $inv_where";
            ...
            $sql = "SELECT i.*, s.supplier_name, 
                    ...
                    LEFT JOIN supplier s ON i.supplier_id = s.supplier_id 
                    $inv_where
                    ORDER BY i.last_service_date DESC, i.date_created DESC
                    ...
                    <table class="table table-hover mb-0">
                        ...
                        <tbody>
                            <?php if ($result && $result->num_rows > 0): ?>
                                ...
                                ?>
                                    <tr>
                                        <td data-label="Item No."><?= htmlspecialchars($row['item_number'] ?? 'N/A') ?></td>
                                        <td data-label="Item Name"><?= htmlspecialchars($row['item_name'] ?? 'N/A') ?></td>
                                        <td data-label="Campus"><?= (!empty($row['campus']) && $row['campus'] !== '0') ? htmlspecialchars($row['campus']) : 'N/A' ?></td>
                                        ...
                                        <td data-label="Actions" class="actions">
                                            ...
                                            <button class="btn btn-sm btn-info" title="Edit"
                                                onclick="openEditPropertyModal(this)"
                                                data-id="<?= (int)$row['id'] ?>"
                                                data-item-number="<?= htmlspecialchars($row['item_number'] ?? '', ENT_QUOTES) ?>"
                                            ...
                                            <button class="btn btn-sm btn-danger" title="Delete"
                                                onclick='deleteProperty(<?= (int)$row['id'] ?>, <?= json_encode($row['brand'] ?? '') ?>)'>
                            ...
                            <?php else: ?>
                                <tr>
                                    <td colspan="14" class="text-center py-4">
                                        ...
                                        <p class="text-muted">No property items found</p>
            ...
            <div class="modal fade" id="addInventoryModal" tabindex="-1">
                <div class="modal-dialog modal-lg">
                    <div class="modal-content">
                        ...
                        <form action="../actions/add_other_property.php" method="POST" enctype="multipart/form-data">
                                ...
                                <div class="mb-3 pb-2 border-bottom">
                                    ...
                                    <div class="row g-3">
                                        <div class="col-md-6">
                                            ...
                                            <input type="text" name="item_number" class="form-control" placeholder="e.g., AC-001">
                                        </div>
                                        <div class="col-md-6">
                                            <label class="form-label">Item Name <span class="text-danger">*</span></label>
                                            <input type="text" name="item_name" class="form-control" required placeholder="e.g., Samsung Split Type">
                                        </div>
                                        <div class="col-md-6">
                    ...
                    <div class="modal-content">
                        <div class="modal-header text-white" style="background-color: var(--primary-green);">
                            <h5 class="modal-title">
                                <i class="fas fa-snowflake me-2"></i>Property Unit Details
                            </h5>
                            ...
                            <div class="row g-4">
                                ...
                                <div class="col-12">
                                    <div class="card border-0 shadow-sm">
                                        ...
                                        <div class="card-body">
                                            <div id="propertyPicturesGallery" class="d-flex flex-wrap gap-2">
                                ...
                                <div class="col-12">
                                    <div class="card border-0 shadow-sm">
                                        ...
                                        <div class="card-body">
                                            <div class="row g-3">
                                                <div class="col-md-4">
                                                    <label class="text-muted small">Property ID</label>
                                                    <p class="fw-bold mb-0" id="view_id">-</p>
                                                </div>
                                                <div class="col-md-4">
                    ...
                    <div class="modal-content">
                        <div class="modal-header" style="background: linear-gradient(135deg, #17a2b8 0%, #138496 100%); color: white;">
                            <h5 class="modal-title">
                                ...
                                <small class="d-block mt-1" style="font-size: 0.85rem;" id="maintenance-property-info"></small>
                            </h5>
                            <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                        </div>
                        <div class="modal-body">
                            ...
                            <div id="noMaintenanceMessage" class="text-center py-4" style="display: none;">
                                ...
                                <p class="text-muted">No maintenance records found for this property unit.</p>
                            </div>
                        </div>
                        <div class="modal-footer">
                            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
                ...
                <div class="modal-dialog modal-lg">
                    <div class="modal-content">
                        ...
                        <form id="maintenanceForm">
                            ...
                            <input type="hidden" id="maint_id" name="id">
                            <div class="modal-body">
                                <div class="row g-3">
                                    <div class="col-md-6">
            ...
            <script>
                ...
                $(document).ready(function() {
                    ...
                    $(document).on('click', '.view-property-details-btn', function() {
                        const button = $(this);
                        ...
                        // Call the viewPropertyDetails function
                        viewPropertyDetails(
                ...
                $(document).ready(function() {
                    $(document).on('click', '.view-maintenance-btn', function() {
                        const button = $(this);
                        currentPropertyId = button.data('aircon-id');
                        const brand = button.data('brand');
                        const model = button.data('model');
                        const serial = button.data('serial');

                        // Set aircon info in modal
                        $('#maintenance-property-info').text(`${brand} ${model} - S/N: ${serial}`);

                        // Show modal
                        $('#maintenanceRecordsModal').modal('show');

                        // Load maintenance records
                        loadMaintenanceRecords(currentPropertyId);
                        ...
                        if (csvLink) csvLink.href = `../actions/export_aircon_maintenance_csv.php?id=${encodeURIComponent(currentPropertyId)}`;
                        if (xlsLink) xlsLink.href = `../actions/export_aircon_maintenance_excel.php?id=${encodeURIComponent(currentPropertyId)}`;
                    });
                    ...
                    $('#addMaintenanceBtn').on('click', function() {
                        ...
                        $('#maint_id').val(currentPropertyId);
                ...
                function performSearch(searchTerm) {
                    ...
                    fetch("other_property_logs.php?" + currentParams.toString())
                ...
                $(document).ready(function() {
                    ...
                    $('#editPropertyModal').on('show.bs.modal', function() {
                        // clearImagePreview('edit'); // Already called in openEditPropertyModal
                    });
                });
                ...
                function fetchItemDetails(inventoryId) {
                    $.ajax({
                        url: '../pages/get_other_property_logs.php',
                        type: 'GET',
                        data: {
                ...
                function editInventoryItem(inventoryId, currentModalId) {
                    ...
                    setTimeout(function() {
                        ...
                        $.ajax({
                            url: '../actions/get_other_property_logs.php',
                            method: 'GET',
                            data: {
                                inventory_id: inventoryId
                            },
        ...
        `;
                ...
                function viewPropertyDetails(airconId, itemNumber, brand, model, type, serialNumber, location, status,
                    ...
                    document.getElementById('view_id').textContent = airconId || 'N/A';
                    ...
                    const gallery = document.getElementById('propertyPicturesGallery');
                    ...
                    fetch('../actions/get_other_property_images.php?id=' + airconId)
                ...
                function printAirconDetails() {
                    const printContent = document.querySelector('#viewPropertyModal .modal-body').innerHTML;
                    ...
                    printWindow.document.write('<h2 class="text-center mb-4">Property Unit Details</h2>');
        ...
        if (!$isAjax) {
            ...
            // Get total number of records
            $count_sql = "SELECT COUNT(*) as total FROM other_property_logs pi $inv_where_ajax";
            $count_result = $conn->query($count_sql);
            $total_records = $count_result->fetch_assoc()['total'];
            $total_pages = ceil($total_records / $records_per_page);
            ...
            $sql = "SELECT pi.*, s.supplier_name 
                    ...
                    LEFT JOIN supplier s ON pi.supplier_id = s.supplier_id 
                    $inv_where_ajax
                    ORDER BY pi.last_service_date DESC, pi.date_created DESC
                    LIMIT $records_per_page OFFSET $offset";
            $result = $conn->query($sql);
            ...
            if ($result && $result->num_rows > 0) {
                ...
                while ($row = $result->fetch_assoc()) {
                    ...
                    echo '<tr>';
                    echo '<td data-label="Item No.">' . htmlspecialchars($row['item_number'] ?? 'N/A') . '</td>';
                    echo '<td data-label="Item Name">' . htmlspecialchars($row['item_name'] ?? 'N/A') . '</td>';
                    echo '<td data-label="Campus">' . ((!empty($row['campus']) && $row['campus'] !== '0') ? htmlspecialchars($row['campus']) : 'N/A') . '</td>';
                    ...
                    echo '<button class="btn btn-sm btn-primary" title="View Details" onclick=\'viewPropertyDetails('
                        . (int)$row['id'] . ', '
                    ...
                    echo '<button class="btn btn-sm btn-info" title="Edit" onclick="openEditPropertyModal(this)" '
                        . 'data-id="' . (int)$row['id'] . '" '
                        . 'data-item-number="' . htmlspecialchars($row['item_number'] ?? '', ENT_QUOTES, 'UTF-8') . '" '
                        ...
                        . 'data-serial-number="' . htmlspecialchars($row['serial_number'] ?? '', ENT_QUOTES, 'UTF-8') . '" '
                        . 'data-location="' . htmlspecialchars($row['location'] ?? '', ENT_QUOTES, 'UTF-8') . '" '
                        . 'data-status="' . htmlspecialchars($row['status'] ?? '', ENT_QUOTES, 'UTF-8') . '" '
                        . 'data-purchase-date="' . htmlspecialchars($row['purchase_date'] ?? '', ENT_QUOTES, 'UTF-8') . '" '
                        . 'data-warranty-expiry="' . htmlspecialchars($row['warranty_expiry'] ?? '', ENT_QUOTES, 'UTF-8') . '" '
                        . 'data-last-service-date="' . htmlspecialchars($row['last_service_date'] ?? '', ENT_QUOTES, 'UTF-8') . '" '
                        . 'data-supplier-id="' . (int)($row['supplier_id'] ?? 0) . '" '
                        . 'data-notes="' . htmlspecialchars($row['notes'] ?? '', ENT_QUOTES, 'UTF-8') . '" '
                        . 'data-purchase-price="' . htmlspecialchars($row['purchase_price'] ?? '0', ENT_QUOTES, 'UTF-8') . '" '
                        . 'data-depreciated-value="' . htmlspecialchars($row['depreciated_value'] ?? '0', ENT_QUOTES, 'UTF-8') . '" '
                        . 'data-campus="' . htmlspecialchars($row['campus'] ?? '', ENT_QUOTES, 'UTF-8') . '" '
                        . 'data-receiver="' . htmlspecialchars($row['receiver'] ?? '', ENT_QUOTES, 'UTF-8') . '"'
                        . '><i class="fas fa-edit"></i></button> ';

                    echo '<button class="btn btn-sm btn-danger" title="Delete" onclick=\'deleteProperty(' . (int)$row['id'] . ', ' . htmlspecialchars(json_encode($row['brand'] ?? ''), ENT_QUOTES, 'UTF-8') . ')\'><i class="fas fa-trash"></i></button></td></tr>';
                }
            } else {
                echo '<tr><td colspan="14" class="text-center py-4"><i class="fas fa-snowflake fa-3x text-muted mb-3"></i><p class="text-muted">No property items found</p></td></tr>';
            }
        ...
        ?>
        <script>
            // Add this to your other_property_logs.php, preferably in the head section or before the closing body tag
            function openEditPropertyModal(btn) {
                // Extract data from button data attributes
                const id = btn.getAttribute('data-id');
                const item_number = btn.getAttribute('data-item-number');
                const item_name = btn.getAttribute('data-item-name');
                const category = btn.getAttribute('data-category');
                const brand = btn.getAttribute('data-brand');
                const model = btn.getAttribute('data-model');
                const type = btn.getAttribute('data-type');
                const serial_number = btn.getAttribute('data-serial-number');
                const location = btn.getAttribute('data-location');
                const status = btn.getAttribute('data-status');
                const purchase_date = btn.getAttribute('data-purchase-date');
                const warranty_expiry = btn.getAttribute('data-warranty-expiry');
                const last_service_date = btn.getAttribute('data-last-service-date');
                const supplier_id = btn.getAttribute('data-supplier-id');
                const notes = btn.getAttribute('data-notes');
                const purchase_price = btn.getAttribute('data-purchase-price');
                const depreciated_value = btn.getAttribute('data-depreciated-value');
                const campus = btn.getAttribute('data-campus');
                const receiver = btn.getAttribute('data-receiver');

                // Format dates for input fields (YYYY-MM-DD)
                const formatDate = (dateString) => {
                    if (!dateString || dateString === '0000-00-00' || dateString === 'N/A') return '';
                    const date = new Date(dateString);
                ...
                document.getElementById('edit_item_number').value = item_number || '';
                ...
                document.getElementById('edit_campus').value = (campus && campus !== '0') ? campus : '';
                ...
                fetch('../actions/get_other_property_images.php?id=' + id)
                    .then(response => response.json())
                    .then(data => {
                            ...
                            data.images.forEach(imgUrl => {
                                ...
                                delBtn.onclick = () => {
                                    if (confirm('Are you sure you want to delete this image?')) {
                                        fetch('../actions/delete_other_property_image.php', {
                                                method: 'POST',
                                                headers: {
                                                    'Content-Type': 'application/x-www-form-urlencoded'
                                                },
                                                body: 'image_path=' + encodeURIComponent(imgUrl) + '&id=' + id
                                            })
                                            .then(r => r.json())
                                            .then(res => {
                                                if (res.success) {
        ...
        <div class="modal fade" id="editPropertyModal" tabindex="-1" aria-hidden="true">
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    ...
                    <form action="../actions/edit_other_property.php" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="modal-body">
                            ...
                            <div class="mb-3 pb-2 border-bottom">
                                ...
                                <div class="row g-3">
                                    <div class="col-md-6">
                                        <label class="form-label">Item Number</label>
                                        <input type="text" name="item_number" id="edit_item_number" class="form-control" placeholder="e.g., AC-001">
                                    </div>
                                    <div class="col-md-6">
                                        <label class="form-label">Item Name <span class="text-danger">*</span></label>
                                        <input type="text" name="item_name" id="edit_item_name" class="form-control" required placeholder="e.g., Samsung Split Type">
                                    </div>
                                    <div class="col-md-6">
                                        <label class="form-label">Category</label>
